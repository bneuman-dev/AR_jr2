...
module Database
  ...
  class Model
    ...
    def self.all
      Model.execute("SELECT * FROM #{self.inspect.downcase}s").map do |row|
        self.new(row)
      end
    end
  ...
  def self.where(query, *args)
    Database::Model.execute("SELECT * FROM #{self.inspect.downcase}s WHERE #{query}", *args).map do |row|
      self.new(row)
    end
  end
  ...
  def self.find(pk)
    ...
    def self.execute(query, *args)
     ...
     self.connection.execute(query, *prepared_args)
    end

    def self.last_insert_row_id
      self.connection.last_insert_row_id
    end
    ...
    def insert!
      ...
      insert_sql = "INSERT INTO #{self.class.inspect.downcase}s (#{fields.join(',')}) VALUES (#{marks})"

      results = Database::Model.execute(insert_sql, *values)

      # This fetches the new primary key and updates this instance
      self[:id] = Model.last_insert_row_id
      results
    end
    ...
    def update!
      ...
      update_sql = "UPDATE #{self.class.inspect.downcase}s SET #{update_clause} WHERE id = ?"
